"""Create a snapshot for a specific VM on Huawei FusionCompute VRM via REST API."""
import argparse
import os
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if sites and "uri" in sites[0]:
                ...
                print("[X] No sites found in the response.")
                return None
        else:
            print(f"[X] Failed to fetch sites. Status code: {response.status_code}")
            return None
    except Exception as e:
...
def create_vm_snapshot(token: str, vm_uri: str, name: str, desc: str) -> Optional[Dict[str, Any]]:
    """
    Creates a snapshot for the specified VM.
    """
    if vm_uri.startswith("/service"):
        ...
        # Normalize urn to uri format if needed, but assuming uri is provided
    ...
    # Some APIs require the exact uri like '/service/sites/urn.../vms/urn...'
    ...
    print(f"\n[*] Creating snapshot '{name}' for VM at {vm_uri}...")
    try:
        response = requests.post(
            snapshot_url, json=payload, headers=headers, verify=False, timeout=30
        )
        if response.status_code in (200, 202):
            data = response.json()
            print("[OK] Snapshot creation command accepted.")
            print(json.dumps(data, indent=2))
            return data
        else:
            print(f"[X] Failed to create snapshot. Status: {response.status_code}")
            print(response.text)
            return None
    except Exception as e:
        print(f"[X] Error creating snapshot: {e}")
        return None


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create a VM snapshot on FusionCompute.")
    parser.add_argument("--vm-name", required=True, help="The name of the VM to snapshot")
    parser.add_argument("--snapshot-name", default=f"snapshot_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}", help="The name of the snapshot")
    parser.add_argument("--snapshot-desc", default="Created by vm_snapshot.py script", help="Description of the snapshot")
    args = parser.parse_args()

    TARGET_VM_NAME = args.vm_name
    SNAPSHOT_NAME = args.snapshot_name
    SNAPSHOT_DESC = args.snapshot_desc
    ...
    if vrm_token:
        ...
        snapshot_data = create_vm_snapshot(vrm_token, vm_uri, SNAPSHOT_NAME, SNAPSHOT_DESC)
        ...
        if snapshot_data:
            ...
            result = {
                'messageId': 'ERROR',
                'messageText': "Failed to create snapshot.",
                'scriptCode': 1,
                'data': {}
            }
            print(json.dumps(result, indent=2))
