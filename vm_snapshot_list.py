"""List snapshots for a specific VM on Huawei FusionCompute VRM via REST API."""
import argparse
import os
...
def get_vrm_token() -> Optional[str]:
    ...
    try:
        # Avoid printing during output generation unless it's for logging 
        # (Since we output JSON at the end, we'll keep print logs simple)
...
def list_vm_snapshots(token: str, vm_uri: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {
                "error": f"Failed to list snapshots. Status: {response.status_code}",
                "details": response.text
            }
    except Exception as e:
        return {"error": f"Error listing snapshots: {e}"}


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="List VM snapshots on FusionCompute.")
    ...
    if vrm_token:
        ...
        if snapshots_data and "error" not in snapshots_data:
            result = {
                'messageId': 'OK',
            ...
            error_text = snapshots_data.get("error", "Failed to list snapshots.") if snapshots_data else "Failed to list snapshots."
            result = {
                'messageId': 'ERROR',
                'messageText': error_text,
                'scriptCode': 1,
                'data': snapshots_data or {}
            }
            print(json.dumps(result, indent=2))
            sys.exit(1)
    else:
